...
const ROLE_NAMES: Record<Role, string> = {
  admin: '管理员', creator: '创意者', supplier: '供应商', user: '普通用户', guest: '访客', suspended: '已禁用'
};
...
function TasksTab() {
  ...
  const fetchOpenDemands = useCallback(async () => {
    setIsLoading(true);
    try {
  const demandsCollection = collection('demands');
  const q = query(demandsCollection, where("status", "==", "开放中"));
      const demandSnapshot = await getDocs(q);
  const demandsList = demandSnapshot.docs.map((doc: any) => ({ id: doc.id, ...doc.data() } as Demand));
...
function SubmissionForm({ imageUrl, onSubmissionSuccess, toolName }: SubmissionFormProps) {
  ...
  await addDoc(collection('products'), {
          ...values,
          imageUrl: publicUrl,
          creatorId: user.uid,
          status: '审核中',
