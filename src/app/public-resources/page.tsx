...
import { collection, getDocs, addDoc, updateDoc, doc, deleteDoc, serverTimestamp, Timestamp } from '@/lib/cloudbase-compat';
...
function ResourceDialog({ resource, open, onOpenChange, onSave }: {
    ...
    const handleSubmit = async (values: z.infer<typeof resourceSchema>) => {
        ...
        try {
            ...
            if (isEditing && resource && resource.id) {
                const docRef = doc('resources', resource.id) as any;
                // Omit createdAt on update
                const { createdAt, ...updateData } = dataToSave as any;
                await updateDoc(docRef as any, updateData as any);
                toast({ title: "成功", description: "数据源已更新。" });
            } else {
                // create new
                await addDoc(collection('resources') as any, dataToSave as any);
...
function ManualResourceManagement() {
    ...
    const fetchResources = useCallback(async () => {
        setIsLoading(true);
        try {
            const resourcesSnapshot = await getDocs(collection('resources'));
            const resourcesList = resourcesSnapshot.docs.map((doc: any) => {
                const data = doc.data();
                return {
                    ...data,
                    id: doc.id,
    ...
    const confirmDelete = async () => {
        ...
        try {
                await deleteDoc(doc('resources', selectedResource.id) as any);
