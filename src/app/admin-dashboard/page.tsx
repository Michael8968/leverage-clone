...
function LlmConnectionForm({ llm, onSave, onCancel, onTest, isTesting }: {
    ...
    useEffect(() => {
        if (form.getValues("modelName") && availableModels.length > 0 && !availableModels.map((m: string) => m.toLowerCase()).includes(form.getValues("modelName").toLowerCase())) {
                            ...
                            <FormField control={form.control} name="provider" render={({ field }) => (
                                <FormItem className="flex flex-col">
                                    ...
                                    <Combobox
                                        ...
                                        options={providers.map((p: any) => ({ value: p.providerName, label: p.providerName }))}
                                        placeholder="选择或输入厂商"
                                    />
                                    <FormMessage />
                                </FormItem>
                           ...
                           <FormField control={form.control} name="modelName" render={({ field }) => (
                                <FormItem className="flex flex-col">
                                    ...
                                    <Combobox
                                        value={field.value}
                                        onChange={field.onChange}
                                        options={availableModels.map((m: string) => ({ value: m, label: m }))}
                                        placeholder="选择或输入模型"
                                    />
                                    <FormMessage />
                                </FormItem>
