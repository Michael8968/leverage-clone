...
const ROLE_NAMES: Record<Role, string> = { admin: '管理员', creator: '创意者', supplier: '供应商', user: '普通用户', guest: '访客', suspended: '已禁用' };
...
function ManualGrantForm({ onSuccessfulGrant }: { onSuccessfulGrant: () => void }) {
        ...
        setIsGranting(async () => {
            try {
                ...
                if (result.userCount > 0) {
                    ...
                    if (currentUser) {
                        try {
                            const meRes = await fetch('/api/auth/me', { headers: { Authorization: `Bearer ${localStorage.getItem('auth_token') || ''}` } });
                            if (meRes.ok) {
                                const { user: fresh } = await meRes.json();
                                setUser(fresh as User, (fresh as User).role);
                            }
                        } catch (e) {
                            // ignore
                        }
    ...
    const targetDescription = () => {
        let parts = [];
        if (targetRoles.length > 0) parts.push(`角色为 "${targetRoles.map((r: Role) => ROLE_NAMES[r] || r).join(', ')}"`);
                        ...
                        <div className="p-3 border rounded-md mt-2 space-y-3">
                            <div>
                                ...
                                <div className="flex flex-wrap gap-x-4 gap-y-2 mt-1">
                                    {ALL_ROLES.map((role: Role) => (<div key={role} className="flex items-center space-x-2"><Checkbox id={`grant-role-${role}`} checked={targetRoles.includes(role)} onCheckedChange={() => handleRoleToggle(role)} /><Label htmlFor={`grant-role-${role}`} className="font-normal">{ROLE_NAMES[role]}</Label></div>))}
                                </div>
                            </div>
                            <div className="pt-3 border-t">
                                <Label className="text-xs text-muted-foreground">按星级筛选</Label>
                                <div className="flex flex-wrap gap-x-4 gap-y-2 mt-1">
                                    {Array.from({ length: 10 }, (_: any, i: number) => i + 1).map((rating: number) => (<div key={rating} className="flex items-center space-x-2"><Checkbox id={`grant-rating-${rating}`} checked={targetRatings.includes(rating)} onCheckedChange={() => handleRatingToggle(rating)} /><Label htmlFor={`grant-rating-${rating}`} className="font-normal">{rating}星</Label></div>))}
                                </div>
                            </div>
                        </div>
                    </div>
...
export default function PointsManagementPage() {
    ...
    const fetchData = useCallback(async () => {
        setIsLoading(true);
        try {
            const [systemRes, roleGiftsRes, pointsRes] = await Promise.all([
                fetch('/api/configs/system'),
                fetch('/api/configs/role_gifts'),
                fetch('/api/configs/points'),
            ]);

            const systemJson = systemRes.ok ? await systemRes.json() : null;
            const roleGiftsJson = roleGiftsRes.ok ? await roleGiftsRes.json() : null;
            const pointsJson = pointsRes.ok ? await pointsRes.json() : null;

            setSystemConfig(systemJson?.data ? (systemJson.data as SystemConfig) : { enable_points: true, enable_payments: true, pro_monthly_bonus: 10000, min_balance_for_llm: 0 });
            setRoleGifts(roleGiftsJson?.data ? (roleGiftsJson.data as RoleGiftsConfig) : { 'user_new': 5000, 'creator_pro': 30000 });
            
            const fetchedPointsConfig = pointsJson?.data ? (pointsJson.data as PointsConfig) : { defaultPricing: {}, rules: {} };
            
            if (!fetchedPointsConfig.rules) {
                fetchedPointsConfig.rules = {};
            }

            ALL_ACTIONS.forEach((action: any) => {
    ...
    const handleSave = async () => {
        ...
        try {
            ...
            await Promise.all([
                fetch('/api/configs/system', { method: 'PUT', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(systemConfig) }),
                fetch('/api/configs/role_gifts', { method: 'PUT', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(roleGifts) }),
                fetch('/api/configs/points', { method: 'PUT', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(finalPointsConfig) }),
            ]);
                    ...
                    <Card>
                        ...
                        <CardContent>
                            <Table>
                                ...
                                <TableBody>
                                    {roleGifts && Object.entries(roleGifts).map(([key, value]: [string, any]) => (
                                        <TableRow key={key}>
                 ...
                 <Card>
                    ...
                    <CardContent className="space-y-4">
                        <Table>
                            ...
                            <TableBody>
                                {pointsConfig && ALL_ACTIONS.map((action: any) => (
                                    <TableRow key={action}>
                                        ...
                                        <TableCell>
                                            {(pointsConfig.rules?.[action] || []).length > 0 ? (
                                                <div className="flex flex-wrap gap-1">
                                                    {(pointsConfig.rules?.[action] || []).map((rule: any) => (
