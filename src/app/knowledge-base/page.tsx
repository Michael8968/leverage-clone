...
import { collection, getDocs, doc, addDoc, updateDoc, deleteDoc, serverTimestamp, query, where } from '@/lib/cloudbase-compat';
...
const knowledgeItemSchema = z.object({
  ...
  price: z.preprocess(
    ...
    z.number({ invalid_type_error: "价格必须是一个数字。" }).nonnegative({ message: "价格不能为负数。" })
  ),
...
function KnowledgeItemDialog({ item, open, onOpenChange, onSave }: {
    ...
    const handleSubmit = async (values: z.infer<typeof knowledgeItemSchema>) => {
        ...
        try {
            if (isEditing) {
                await updateDoc(doc('products', item!.id!), { ...values });
                toast({ title: "成功", description: "知识条目已更新。" });
            } else {
                await addDoc(collection('products'), { ...values, createdAt: serverTimestamp() });
...
function KnowledgeBaseList() {
    ...
    const fetchKnowledgeItems = useCallback(async () => {
        ...
        try {
            ...
            const productsList = productSnapshot.docs.map((doc: any) => {
                const data = doc.data();
                return {
                    ...data,
                    id: doc.id,
    ...
    const confirmDelete = async () => {
        ...
        try {
            await deleteDoc(doc('products', selectedItem.id));
...
function ApiDataFetcher() {
    ...
    const fetchAvailableResources = async () => {
            setIsLoading(true);
            try {
        const resourcesCollection = collection('resources');
                const q = query(resourcesCollection, where("status", "==", "可用"));
                const snapshot = await getDocs(q);
                const resourcesList = snapshot.docs.map((doc: any) => ({ id: doc.id, ...doc.data() } as Resource));
