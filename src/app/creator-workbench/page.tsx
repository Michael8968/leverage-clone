...
import { collection, getDocs, query, where, doc, updateDoc, addDoc, serverTimestamp, getDoc, deleteDoc, TcbTimestamp, setDoc, orderBy, arrayUnion, arrayRemove } from '@/lib/cloudbase-compat';
...
function TasksTab() {
  ...
  const { user } = useAuthStore();
    ...
    const fetchOpenDemands = useCallback(async () => {
        ...
        try {
            ...
            // 客户端过滤 status
            const demandsList = demandsSnapshot.docs
                .map((doc: any) => ({ id: doc.id, ...doc.data() } as Demand))
                .filter((d: any) => d.status === '开放中');
  ...
  useEffect(() => {
    ...
    const handleAcceptTask = async (demandId: string) => {
        ...
        try {
            const demandRef = doc('demands', demandId);
            await updateDoc(demandRef as any, {
                status: "进行中",
                creatorId: user.uid,
            });
    ...
    useEffect(() => {
        const fetchSubmissions = async () => {
            ...
            try {
                ...
                const snapshot = await getDocs(q as any);
                let subsList = snapshot.docs.map((doc: any) => {
                    const data = doc.data();
                    // CloudBase 兼容层时间戳处理
                ...
                // 客户端排序
                subsList.sort((a: any, b: any) => (b.createdAt?.getTime() || 0) - (a.createdAt?.getTime() || 0));
                 ...
                 <Table>
                    ...
                    <TableBody>
                        {isLoading ? (
                            ...
                            submissions.map((sub: ProductService) => (
                                <TableRow key={sub.id}>
                                    <TableCell>
...
function ScheduleAndAssistantTab() {
    ...
    const fetchCreatorData = useCallback(async () => {
        ...
        try {
            ...
            const availablePrompts = promptsData.prompts.filter((p: any) => p.ownerType === 'platform' || p.ownerId === user.uid);
            setPrompts(availablePrompts as any);

            // Fetch appointments
            const apptQuery = query(collection('appointments'), where("creatorId", "==", user.uid));
            const apptSnapshot = await getDocs(apptQuery as any);
          const apptList = apptSnapshot.docs.map((doc: any) => {
              const data = doc.data();
                 // CloudBase 兼容层时间戳处理
                 return { 
                    ...data, 
                    id: doc.id,
                    appointmentTime: (data.appointmentTime as any)?.toDate ? (data.appointmentTime as any).toDate() : new Date(data.appointmentTime),
                    createdAt: (data.createdAt as any)?.toDate ? (data.createdAt as any).toDate() : new Date(data.createdAt),
                } as Appointment;
            });
            apptList.sort((a: any,b: any) => new Date(b.appointmentTime).getTime() - new Date(a.appointmentTime).getTime());
    ...
    const handleAppointmentStatus = async (appointmentId: string, status: 'confirmed' | 'cancelled') => {
        try {
            await updateDoc(doc('appointments', appointmentId) as any, { status });
    ...
    const newSlotTimestamp = (TcbTimestamp && (TcbTimestamp as any).fromDate) ? (TcbTimestamp as any).fromDate(newSlotDate as Date) : newSlotDate;
        try {
            const availRef = doc('availabilities', user.uid);
            await updateDoc(availRef as any, { slots: arrayUnion(newSlotTimestamp) });
            ...
            if ((error as any).code === 'not-found') {
                try {
                    await setDoc(doc('availabilities', user.uid) as any, { slots: [newSlotTimestamp] });
    ...
    const handleDeleteSlot = async (slot: any) => {
        ...
        try {
            const availRef = doc('availabilities', user.uid);
            await updateDoc(availRef as any, { slots: arrayRemove(slot) });
            setAvailableSlots(prev => prev.filter(s => { try { return (s as any).toMillis() !== (slot as any).toMillis(); } catch { return JSON.stringify(s) !== JSON.stringify(slot); } }));
     ...
     useEffect(() => {
        if (open && user) {
            ...
            const fetchHistory = async () => {
                try {
                    const q = query(collection('points_transactions'), where('uid', '==', user.uid));
                    const snapshot = await getDocs(q as any);
                    ...
                    // 客户端排序
                    history.sort((a: any, b: any) => (b.timestamp?.getTime() || 0) - (a.timestamp?.getTime() || 0));
