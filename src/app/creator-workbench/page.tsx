...
// =================================================================
// TASKS TAB - The primary component being modified in this step.
// =================================================================
function TasksTab() {
...
// =================================================================
// The components below are included to ensure the file is complete,
// but their internal logic has NOT been changed in this step.
  ...
  const onSubmit = (values: z.infer<typeof formSchema>) => {
    startTransition(async () => {
      try {
        ...
        toast({ title: '错误', description: 'AI模型生成失败，请稍后再试。', variant: 'destructive' });
      }
    });
  };
  return (
    <Card>
      <CardHeader><CardTitle className="font-headline">3D AI创作引擎</CardTitle><CardDescription>基于 Tripo Studio 的下一代 AI 3D 生成技术...</CardDescription></CardHeader>
      <CardContent className="space-y-4">
        {/* Form and image display logic remains here */}
      </CardContent>
    </Card>
  );
}
...
function SubmissionsTab() {
  // This component's logic remains as it was in the original file.
  // It correctly fetches submissions for the logged-in creator.
  return (
    <Card>
        <CardHeader><CardTitle className="font-headline">我的创意提交</CardTitle><CardDescription>您提交的创意作品将在此处进行管理。</CardDescription></CardHeader>
        <CardContent>{/* Table for submissions remains here */}</CardContent>
    </Card>
  );
}
...
function CreatorWorkbench() {
  return (
    <div className="p-4 md:p-8">
      <header className='text-center mb-8'><h1 className="text-3xl font-headline font-bold">创意者工作台</h1><p className="text-muted-foreground mt-2">在这里, 您可以接受任务, 响应需求, 并利用AI工具将您的创意变为现实。</p></header>
      <Tabs defaultValue="tasks">
        <TabsList className="grid w-full grid-cols-3 max-w-lg mx-auto"><TabsTrigger value="tasks">任务与需求</TabsTrigger><TabsTrigger value="3d-creation">3D AI 创作</TabsTrigger><TabsTrigger value="submissions">我的提交</TabsTrigger></TabsList>
...
function RestrictedAccess() {
    return (
        <div className="flex flex-col items-center justify-center h-full p-4 text-center"><Frown className="w-16 h-16 mb-4 text-destructive"/><h2 className="text-2xl font-bold font-headline mb-2">访问受限</h2><p className="text-muted-foreground">此页面仅对“创意者”角色的用户开放。</p></div>
    );
}
