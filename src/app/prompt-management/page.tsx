...
import { collection, getDocs, doc, updateDoc, addDoc, serverTimestamp, deleteDoc, Timestamp } from '@/lib/cloudbase-compat';
...
export default function PromptManagementPage() {
    ...
    const fetchData = useCallback(async () => {
        ...
        setIsLoading(true);
        try {
            // Fetch prompts and perform role-based client-side filtering/sorting
            const promptsSnapshot = await getDocs(collection('prompts'));
            const promptsList = promptsSnapshot.docs.map((d: any) => {
                const data = d.data();
                // normalize createdAt (CloudBase may return Date-like objects)
            ...
            const filteredByRole = role === 'admin' ? promptsList : promptsList.filter((p: any) => p.ownerId === user.uid);
            filteredByRole.sort((a: any, b: any) => a.name.localeCompare(b.name));
            setPrompts(filteredByRole);

            // Fetch active LLMs for the dropdown
            const llmsSnapshot = await getDocs(collection('llm_connections'));
            const allLlms = llmsSnapshot.docs.map((d: any) => ({ ...d.data(), id: d.id } as LlmConnection));
            // sort by priority client-side
                allLlms.sort((a: LlmConnection, b: LlmConnection) => (a.priority || 0) - (b.priority || 0));
            setLlms(allLlms.filter((llm: any) => llm.status === '活跃'));
    ...
    const confirmDelete = async () => {
        ...
        try {
            await deleteDoc(doc('prompts', itemToDelete.id));
                ...
                <Card>
                    ...
                    <CardContent>
                         <Table>
                            ...
                            <TableBody>
                                {isLoading ? Array.from({ length: 3 }).map((_: any, i: number) => (
...
`;
    ...
    const handleSubmit = async (values: z.infer<typeof promptSchema>) => {
        ...
        try {
            ...
            if (isEditing) {
                const docRef = doc('prompts', prompt!.id!);
                ...
                await addDoc(collection('prompts'), { 
                    ...dataToSave, 
                    promptKey: finalKey,
                    ownerId: user.uid,
                                    ...
                                    <FormLabel>绑定模型</FormLabel>
                                     ...
                                     >
                                        ...
                                        <SelectContent>
                                            ...
                                            {llms.map((llm: any) => <SelectItem key={llm.id} value={llm.id}>{llm.modelName} ({llm.provider})</SelectItem>)}
                                        </SelectContent>
                                    </Select>
                                    <FormMessage />
                                </FormItem>
