...
import { doc, getDoc, setDoc, serverTimestamp } from '@/lib/cloudbase-compat';
import { TcbTimestamp, Timestamp as CBTimestamp } from '@/lib/cloudbase-compat';
// db removed; using cloudbase-compat collection/doc helpers
...
function StrategyRuleDialog({
    ...
    const handleActionTemperatureChange = (factorId: string, value: number) => {
        ...
        setRule(prev => {
            const newActions = prev.actions.type === 'apply_weights' ? {
                ...prev.actions,
                factorTemperatures: {
                    ...((prev.actions as any).factorTemperatures || {}),
                    [factorId]: value,
                }
            } : prev.actions;
                                                ...
                                                <div className="p-4 border rounded-md space-y-4"><Label>绝对时间范围 (一次性生效)</Label><div className="grid grid-cols-2 gap-4">
                                                    <Popover>
                                                        <PopoverTrigger asChild>
                                                            <Button variant={"outline"} className={cn("w-full justify-start text-left font-normal", !conditions.startsAt && "text-muted-foreground")}>
                                                                <CalendarIcon className="mr-2 h-4 w-4" />
                                                                {conditions.startsAt ? format(((conditions.startsAt as any)?.toDate ? (conditions.startsAt as any).toDate() : new Date(conditions.startsAt as any)), "yyyy-MM-dd HH:mm") : <span>选择生效时间</span>}
                                                            </Button>
                                                        </PopoverTrigger>
                                                        <PopoverContent className="w-auto p-0">
                                                            <Calendar mode="single" selected={conditions.startsAt ? ((conditions.startsAt as any)?.toDate ? (conditions.startsAt as any).toDate() : undefined) : undefined} onSelect={d => handleConditionChange('startsAt', d ? ((CBTimestamp && (CBTimestamp as any).fromDate) ? (CBTimestamp as any).fromDate(d) : d) : undefined)} initialFocus />
                                                            <div className="p-3 border-t border-border"><TimePicker date={conditions.startsAt ? ((conditions.startsAt as any)?.toDate ? (conditions.startsAt as any).toDate() : undefined) : undefined} setDate={d => handleConditionChange('startsAt', d ? ((CBTimestamp && (CBTimestamp as any).fromDate) ? (CBTimestamp as any).fromDate(d) : d) : undefined)} /></div>
                                                        </PopoverContent>
                                                    </Popover>
                                                    <Popover>
                                                        <PopoverTrigger asChild>
                                                            <Button variant={"outline"} className={cn("w-full justify-start text-left font-normal", !conditions.expiresAt && "text-muted-foreground")}>
                                                                <CalendarIcon className="mr-2 h-4 w-4" />
                                                                {conditions.expiresAt ? format(((conditions.expiresAt as any)?.toDate ? (conditions.expiresAt as any).toDate() : new Date(conditions.expiresAt as any)), "yyyy-MM-dd HH:mm") : <span>选择失效时间</span>}
                                                            </Button>
                                                        </PopoverTrigger>
                                                        <PopoverContent className="w-auto p-0">
                                                            <Calendar mode="single" selected={conditions.expiresAt ? ((conditions.expiresAt as any)?.toDate ? (conditions.expiresAt as any).toDate() : undefined) : undefined} onSelect={d => handleConditionChange('expiresAt', d ? ((CBTimestamp && (CBTimestamp as any).fromDate) ? (CBTimestamp as any).fromDate(d) : d) : undefined)} />
                                                            <div className="p-3 border-t border-border"><TimePicker date={conditions.expiresAt ? ((conditions.expiresAt as any)?.toDate ? (conditions.expiresAt as any).toDate() : undefined) : undefined} setDate={d => handleConditionChange('expiresAt', d ? ((CBTimestamp && (CBTimestamp as any).fromDate) ? (CBTimestamp as any).fromDate(d) : d) : undefined)} /></div>
                                                        </PopoverContent>
                                                    </Popover>
                                                </div></div>
                                            )}
                                                ...
                                                {ALL_ROLES.map((role: Role) => (
                                                    <div key={role} className="p-3 border rounded-md">
                                                        <div className="flex items-center space-x-2">
                                                            <Checkbox id={`role-${role}`} checked={!!conditions.targetUserRoles?.[role]} onCheckedChange={() => handleRoleToggle(role)} />
                                                            <Label htmlFor={`role-${role}`} className="text-sm font-medium">{ROLE_NAMES[role]}</Label>
                                                        </div>
                                                        {conditions.targetUserRoles?.[role] && (
                                                                ...
                                                                <div className="flex flex-wrap gap-x-3 gap-y-1">
                                                                    {Array.from({length: 10}, (_: any, i: number) => i + 1).map((rating: number) => (
                                                                        <div key={rating} className="flex items-center space-x-1">
                                                                            <Checkbox id={`rating-${role}-${rating}`} checked={conditions.targetUserRoles?.[role]?.includes(rating)} onCheckedChange={() => handleRatingToggle(role, rating)} />
...
function FactorManagementDialog({
    ...
    const handleFactorChange = (id: string, field: keyof DecisionFactor, value: string) => {
        setCurrentFactors(currentFactors.map((f: any) => f.id === id ? { ...f, [field]: value } : f));
    }
...
export default function IntelligentRoutingModulePage() {
    ...
    const handleSave = async () => {
        ...
        try {
            ...
            factors.forEach((factor: any) => { finalTemperatures[factor.id] = factorTemperatures[factor.id] ?? 0.5; });
    ...
    const handleFactorsChange = (newFactors: DecisionFactor[]) => {
        ...
        newFactors.forEach((factor: any) => { newTemperatures[factor.id] = factorTemperatures[factor.id] ?? 0.5; });
                                    ...
                                    <Table>
                                        ...
                                        <TableBody>
                                            {advancedRules.length === 0 ? (<TableRow><TableCell colSpan={4} className="h-24 text-center">暂无高级策略。</TableCell></TableRow>) : (
                                                advancedRules.sort((a: any, b: any) => a.priority - b.priority).map((rule: any) => (
