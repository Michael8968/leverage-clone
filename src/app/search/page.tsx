...
import { generateUserProfile } from '@/ai/flows/user-profiling';
...
function SearchPageComponent() {
  ...
  useEffect(() => {
    startInitialLoad(async () => {
        try {
            const productsQuery = await getDocs(collection('products'));
            const productsData = productsQuery.docs.map((doc: any) => ({ id: doc.id, ...doc.data() } as ProductService));
            setAllProducts(productsData);
            const suppliersQuery = await getDocs(collection('suppliers'));
            const suppliersData = suppliersQuery.docs.map((doc: any) => ({ id: doc.id, ...doc.data() } as Supplier));
  ...
  const renderContent = () => {
    ...
    return (
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 mt-6">
            {sortedResults.map((product: ProductService) => (
