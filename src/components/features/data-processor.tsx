...
export function DataProcessor({ className, destination = 'suppliers' }: { className?: string, destination?: 'suppliers' | 'products' | 'resources' }) {
  ...
  const saveData = async () => {
    ...
    const targetCollectionName = destination;
        
    aiResult.forEach((itemData: any) => {
      // Create a temporary ID for batch processing
      let dataToSave: any = {
        ...itemData,
      ...
      // In our compat layer, batch.set expects a docRef-like object; we'll generate a temporary id
      const tempId = `tmp_${Date.now()}_${Math.random().toString(36).slice(2,8)}`;
      batch.set({ _col: targetCollectionName, _id: tempId }, dataToSave);
      });
